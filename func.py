...
def clean_up (text):
  ...
  """Функция удаляет из текста спецсимволы, фигурные скобки и ссылки"""

  text = text.replace('\n', ' ').replace('\t', ' ').replace('\xa0', ' ').replace('<br>', ' ').replace('nobr', ' ').replace('|', ', ')
  text=text.replace('{{', ' ').replace('}}', ' ').replace('[[', ' ').replace(']]', ' ').replace('!', ' ').replace("'''", "'").replace('==', ' ')
  text=text.replace('<onlyinclude>', ' ').replace('</onlyinclude>', ' ').replace('<noinclude>', ' ').replace('</noinclude>', ' ').replace('<blockquote>', ' ').replace('</blockquote>', ' ')
  text = text.replace('lang-pt,', ' ').replace('lang-la', ' ').replace('ref-la', ' ')

  text = re.sub(r'<ref.*</ref>', ' ', text)
  text = re.sub(r'\burl=http\S+\s*', ' ', text)
  text = re.sub(r'\bhttp\S+\s*', ' ', text)
  #Строка ниже удалит всё, что идет ниже раздела "Примечания", т.е. оставит только текст статьи!!!
  text = re.split('Примечания', text)[0]
...
def enciclopedic(text, tokenizer, model, prompt):

    '''Функция проверяет соответсвие энциклопедическому стилю,
    с использованием прдобученной модели Bert'''
    ...
    if predicted_class == 1:
        return "Текст написан в энциклопедическом стиле"
    else:
        return "Текст написан не в энциклопедическом стиле"



#Соответствие Wiki-разметке
def is_mediawiki_markup(text):
    """
    Определяет, соответствует ли текст разметке MediaWiki.

    :param text: проверяемый текст
    :return: True, если текст соответствует разметке MediaWiki, иначе False
    """
    wiki_text = parse(text)
    templates = wiki_text.filter_templates()
    sections = wiki_text.get_sections()
    links = wiki_text.filter_wikilinks()

    if templates and sections and links:
        return "Соответствует Wiki-разметке"
    else:
        return "Не соответствует Wiki-разметке"


"""Нейтральность"""

def get_sentiment(text, tokenizer, model, return_type='label'):
